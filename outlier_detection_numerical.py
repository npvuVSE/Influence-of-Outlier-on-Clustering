...
#         print(f'{percentage}\\% & \\{method_name} & {accuracy * 100:.2f} & {precision * 100:.2f} \\\\')
...
results = {method: {} for method in outlier_detection_methods}

for run in runs:
    for name in outlier_names:
        for percentage in outlier_percentages:
            filepath = f'data/numerical/wOutliers/run{run}/df_{name}_outliers_{percentage}percent.csv'
            df = pd.read_csv(filepath)
            
            for method_name, method in outlier_detection_methods.items():
                result_key = f'{name}_{percentage}'
                if result_key not in results[method_name]:
                    results[method_name][result_key] = {'accuracy': [], 'precision': []}
                
                X, y = split_df(df, number_of_columns=2)
                percentage_float = percentage / 100 - 0.009
                outliers = method(X, contamination=percentage_float)

                directory = f'data/numerical/wOutliers/run{run}/removed/{method_name}'
                if not os.path.exists(directory):
                    os.makedirs(directory)
                
                df_no_outliers = df[~np.array(outliers)]
                df_no_outliers.to_csv(f'{directory}/df_{name}_outliers_{percentage}percent_removed_{method_name}.csv', index=False)
                
                true_outliers = y['IsOutlier']
                accuracy = accuracy_score(true_outliers, outliers)
                precision = precision_score(true_outliers, outliers, zero_division=0)
                
                results[method_name][result_key]['accuracy'].append(accuracy)
                results[method_name][result_key]['precision'].append(precision)

mean_results = {method: {} for method in outlier_detection_methods}
for method, data in results.items():
    for key, metrics in data.items():
        mean_accuracy = np.mean(metrics['accuracy']) * 100
        mean_precision = np.mean(metrics['precision']) * 100
        mean_results[method][key] = {
            'mean_accuracy': mean_accuracy,
            'mean_precision': mean_precision
        }
        print(f"{method} - {key} - Mean Accuracy, Mean Precision: {mean_accuracy:.2f} & {mean_precision:.2f}")

for method, res in mean_results.items():
    print(f"Results for {method}:")
    mean_results_df = pd.DataFrame.from_dict(res, orient='index')
